...
test_that("score_custom works", {
  score_custom(
    ...
    expect_equal(c(
      rep(NA, 9), 0:10 / 20 + 0.4, 0.9 - 1:30 / (30 / 0.9), 1:10 / 50,
...
test_that("transform_exponential works", {
  ...
  transform_exponential(x, exponential = T, logarithmic = T, magnitude = 1) %>%
    expect_equal(transform_exponential(x,
      exponential = T, logarithmic = F,
      magnitude = -1
    ))
})
...
test_that("score_column works", {
  score_column(NULL) %>%
    expect_equal(NA)
  score_column(1:100,
    score_type = NA_character_, lb = 0, ub = 100, centre = NA,
    inverse = NA,
    exponential = F, logarithmic = NA, magnitude = NA, custom_args = NA
  ) %>%
    expect_equal(1:100 / 100)
  score_column(1:100,
    score_type = "Linear", lb = 0, ub = 100, centre = NA,
    inverse = NA,
    exponential = F, logarithmic = NA, magnitude = NA, custom_args = NA
  ) %>%
    expect_equal(1:100 / 100)
