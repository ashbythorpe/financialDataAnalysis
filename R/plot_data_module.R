...
plot_data_ui <- function(id) {
  ...
  div(
    ...
    box(
      ...
      conditionalPanel(
        "input.plot_select == 'Score performance'",
        ns = ns,
        score_performance_ui(ns("score_performance"))
      ),
      conditionalPanel(
        "input.plot_select == 'Correlation heatmap'",
        ns = ns,
        correlation_plot_ui(ns("correlation_plot"))
      ),
      conditionalPanel(
        "input.plot_select == 'Custom'",
        ns = ns,
...
plot_data_server <- function(id, data, scores, interactive) {
  moduleServer(id, function(input, output, session) {
    ...
    plot_server(
      "plot",
      type = reactive(input$plot_select), data = data, scores = scores,
