    ...
    tabsetPanel(
      ...
      tabPanelBody(
        ...
        shinyWidgets::airDatepickerInput(
          ns("dates_daily"),
          label = "Prediction range", range = TRUE,
      ...
      tabPanelBody(
        ...
        shinyWidgets::airMonthpickerInput(
          ns("dates_monthly"),
          range = TRUE, label = "Prediction range",
  ...
  moduleServer(id, function(input, output, session) {
    ...
    plot <- reactive({
      ...
      if (input$frequency == "daily") {
        ...
        preds <- predict_price(
          stock(),
          start_date = from, end_date = to, freq = "daily",
          hostess = hostess
        )
      } else {
        req(
          length(input$dates_monthly) == 2,
        ...
        preds <- predict_price(
          stock(),
          start_date = from, end_date = to, freq = "monthly",
          hostess = hostess
        )
      }
    ...
    observe({
      ...
      bindEvent({
        req(plot(), !identical(interactive_value(), FALSE))
        plotly::event_data("plotly_afterplot",
          source = "predict",
          priority = "event"
        )
      })
